...
export const sortPaddockTypeByTotalArea = () => {
  const paddockAcc = paddocks.reduce(
    (results, current) => ({
      ...results,
      [current.paddockTypeId]: {
        name: paddockType[current.paddockTypeId - 1].name,
        totalArea:  current.area + (results[current.paddockTypeId] ? results[current.paddockTypeId].totalArea : 0),
      },
    }), {}
  );

  return Object.values(paddockAcc)
...
export const sortFarmManagerByAdminArea = () => {
  ...
  const paddockAcc = paddocks.reduce(
    (results, current) => ({
      ...results,
      [current.paddockManagerId]: {
        id: current.paddockManagerId,
        name: paddockManagers[current.paddockManagerId-1].name,
        totalArea:  current.area + (results[current.paddockManagerId] ? results[current.paddockManagerId].totalArea : 0),
      },
    }), {}
  );
...
export const farmManagerNames = () => {
  ...
  const paddockAcc = paddocks.reduce(
    (results, current) => ({
      ...
      [current.farmId]: {
        id: current.farmId,
        managgers: (results[current.farmId] ? results[current.farmId].managgers : new Set()).add(current.paddockManagerId)
      },
    }), {}
  );
